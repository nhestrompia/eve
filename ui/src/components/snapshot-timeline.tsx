  ...
  return (
    ...
    >
      <div className="relative flex justify-center">
        ...
        >
          {isSelected ? <CheckCircle2 className="size-3.5" /> : <GitCommitHorizontal className="size-3 text-slate-500" />}
        </span>
        {!isLast ? <span className="absolute top-7 h-[calc(100%+4px)] w-px bg-slate-200" /> : null}
      </div>
      <div className="min-w-0">
        <div className="flex items-center justify-between gap-3">
          <p className="truncate font-medium">{evolution.title || 'Untitled Evolution'}</p>
          <code className={cn('shrink-0 font-mono text-xs tabular-nums text-muted-foreground', isSelected && 'text-blue-700')}>
            {evolution.id.replace(/^EV-/, '#')}
          </code>
        </div>
        <div className="mt-1 flex items-center justify-between gap-3 text-xs text-muted-foreground">
          <code className="truncate rounded-md bg-secondary px-1.5 py-0.5 font-mono tabular-nums">{shortCommit(evolution.snapshot)}</code>
          <span className="shrink-0">{compactDate(evolution.updatedAt || evolution.createdAt)}</span>
        </div>
      </div>
    </div>
  );
