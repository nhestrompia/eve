...
export function SnapshotCodeBrowser({
  snapshotId,
  repository,
  className
}: {
  ...
  const files = useQuery({
    queryKey: ['snapshot-code-files', snapshotId, repository],
    queryFn: () => api.snapshotCodeFiles(snapshotId, repository),
    enabled: Boolean(snapshotId && repository)
  });
  const [showChanged, setShowChanged] = useState(false);
  const [selectedPath, setSelectedPath] = useState<string | undefined>();
  const [mode, setMode] = useState<SnapshotCodeFileMode>('diff');

  const allFiles = useMemo(() => files.data?.files ?? [], [files.data]);
  const curatedFiles = useMemo(() => allFiles.filter((file) => file.curated), [allFiles]);
  const changedFiles = useMemo(() => allFiles.filter((file) => !file.curated), [allFiles]);
  ...
  const fileContent = useQuery({
    queryKey: ['snapshot-code-file', snapshotId, repository, selectedPath, mode],
    queryFn: () => api.snapshotCodeFile(snapshotId, selectedPath ?? '', mode, repository),
    enabled: Boolean(snapshotId && repository && selectedPath && selectedFile?.previewable)
  });

  if (!repository || files.isLoading) {
    return <LoadingState label="Loading code" />;
  }

  if (files.error) {
    return <ErrorState error={files.error} />;
  }

  if (allFiles.length === 0) {
    return <EmptyCodePanel text="No changed files were found for this Snapshot." />;
  }

  return (
    <div className={cn('code-tab-shell grid min-h-[620px] grid-cols-1 overflow-hidden rounded-lg border bg-card lg:grid-cols-[340px_minmax(0,1fr)]', className)}>
      <CodeFileList
...
function EmptyCodePanel({ text }: { text: string }) {
