    ...
    <RailCard title="Evolution velocity" eyebrow="Last 12 months">
      <div className="space-y-4">
        {repositories.slice(0, 4).map((repo, index) => {
          ...
          const values = repositoryVelocityBuckets(repo, evolutions, repositories);
          ...
          >
            ...
            <span className="h-1.5 overflow-hidden rounded-full bg-slate-100">
              <span
                ...
                style={{ width: row.count > 0 ? `${Math.max(7, (row.count / max) * 100)}%` : "0%" }}
              />
            </span>
...
function evolutionBelongsToRepository(
  ...
  if (evolution.repository) {
    return evolution.repository === repo.name || evolution.repository === repo.id;
  }
...
function compressSeries(values: number[], length: number) {
  ...
  return Array.from({ length }, (_, index) => {
    ...
    const end = Math.max(start + 1, Math.floor(((index + 1) / length) * values.length));
