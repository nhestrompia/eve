...
func TestRunHelp(t *testing.T) {
	...
	for _, want := range []string{"Usage: eve", "Run `eve` without a command", "Commands:"} {
...
func TestRunLaunchRejectsNonLocalhostAddress(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"--addr", "0.0.0.0:4317"}, &stdout, &stderr)
	if code != 2 {
		t.Fatalf("exit code = %d, want 2", code)
	}
	if !strings.Contains(stderr.String(), "eve only binds to localhost") {
		t.Fatalf("stderr = %q, want localhost warning", stderr.String())
	}
}

func TestRunKillRejectsNonLocalhostAddress(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := run([]string{"kill", "--addr", "0.0.0.0:4317"}, &stdout, &stderr)
	if code != 2 {
		t.Fatalf("exit code = %d, want 2", code)
	}
	if !strings.Contains(stderr.String(), "eve kill only targets localhost") {
		t.Fatalf("stderr = %q, want localhost warning", stderr.String())
	}
}

func TestParsePIDLines(t *testing.T) {
	got := parsePIDLines("123\nnot-a-pid\n456\n")
	if fmt.Sprint(got) != "[123 456]" {
		t.Fatalf("parsePIDLines = %#v, want [123 456]", got)
	}
}
